...
# Add a button to show more files
num_files_to_show = 5
def show_more_files():
    global num_files_to_show
    num_files_to_show += 5
    refresh_files()

show_more_button = tk.Button(window, text='Show More', command=show_more_files)
...
def refresh_files():
    last_modified_files = get_last_modified_files(num_files_to_show)
    if last_modified_files:
        ...
        if len(last_modified_files) < num_files_to_show:
